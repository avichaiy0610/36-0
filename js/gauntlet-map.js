...
// Every line of copy on a road card is editable from the admin panel: the same
// site-text lookup the rest of the site uses, keyed by club and season.
function gmText(prefix, node, fallback) {
...
function gmRoadPicker(at, over) {
  ...
  return `
    <div class="gm-picker">
      ${row.act ? `<div class="gm-picker-act">${gmActText(row.act)}</div>` : ''}
      ...
      ${row.nodes.map((n, ni) => {
        ...
        const elite = typeof gtIsElite === 'function' && gtIsElite(n);
        const desc = gmText('gt-desc-', n, n.desc);
        ...
        <button class="gm-road ${elite ? 'elite' : ''}" data-row="${at}" data-node="${ni}">
          ...
          <span class="gm-road-main">
            ...
            ${n.fin ? `<span class="gm-road-fin">${gmText('gt-fin-', n, n.fin)}</span>` : ''}
...
const GM_RUN = [
  { kind: 'fight', round: 1, act: 'התחתית', nodes: [
    ...
    { teamId: 'hapoel-pt',         season: '2006/07', ovr: 76, tag: 'down', fin: 'מקום 12/12 · 3 ניצחונות',
      desc: 'שלושה ניצחונות בעונה שלמה, ומקום אחרון בליגה של 12.' },
  ...
  { kind: 'fight', round: 3, act: 'אמצע הטבלה', nodes: [
    ...
    { teamId: 'ironi-ks',          season: '2011/12', ovr: 82, tag: 'champ', fin: 'אלופה · 73 נק׳',
      desc: 'האליפות היחידה של קריית שמונה: 21 ניצחונות, 26 ספיגות, ושמעון אבוחצירא (85) עם 13 שערים.' },
  ...
  { kind: 'fight', round: 6, act: 'המתמודדות', nodes: [
    { teamId: 'hapoel-beersheba',  season: '2025/26', ovr: 85, tag: 'champ', fin: 'אלופה · 79 נק׳',
      desc: 'האליפות האחרונה בדאטה: 79 נקודות, 79 שערים, ודן ביטון עם 15.' },
