...
Deno.serve(async (req) => {
  ...
  // ── דירוגי הקהל והצמדים ───────────────────────────────────────────────────
  // אותו בוט, אותו וובהוק. לבוט אחד יכול להיות וובהוק אחד בלבד, אז התוספת
  // הזאת יושבת כאן ולא בפונקציה נפרדת, ומנותבת לפי הקידומת.
  //
  // הכפתור נושא מזהה קצר ולא את ההחלטה עצמה: callback_data מוגבל ל-64 בייטים
  // ושם שחקן בעברית הוא כשני בייטים לתו. ההחלטה המלאה יושבת ב-engine_state,
  // ששם scripts/crowd_telegram.js הניח אותה כששלח את ההודעה.
  //
  // ולמה כאן כותבים לטבלאות ישירות ולא קוראים ל-approve_rating: ה-RPC-ים
  // מגודרים ב-is_site_admin(), שבודק אימייל בתוך ה-JWT, ול-service_role אין
  // אימייל. הרחבת השער הייתה נוגעת בפרימיטיב שכל הסכימה נשענת עליו. במקום זה
  // השולח — שרץ עם אותו מפתח ויכול לקרוא הכל — כבר אימת את הפריט וחישב את
  // ההחלטה, וכאן נשארת כתיבה אחת. אותו דפוס בדיוק כמו challenge_overrides למעלה.
  if (action === 'cw') {
    const id = period, verb = key;      // cw:<id>:<a|p|x>
    let msg = 'הפריט לא נמצא (אולי כבר טופל)';
    try {
      const raw = await stateGet(`crowd_pending|${id}`);
      if (!raw) {
        console.log(`tap: nothing pending for crowd id ${id}`);
      } else {
        ...
        if (p.t === 'r' && verb === 'a') {
          ...
          await sb('POST', 'duo_approvals', { pair_key: p.p, votes: p.n },
            { Prefer: 'resolution=merge-duplicates' });
          msg = `✅ ${p.p.replace('|', ' + ')} · ממתין ל-apply_crowd_duos`;
        } else if (p.t === 'd') {
          await sb('POST', 'duo_dismissals', { pair_key: p.p, at_votes: p.n },
            { Prefer: 'resolution=merge-duplicates' });
          msg = `🗑️ ${p.p.replace('|', ' + ')} ירד מהתור`;
        }
        // מחרוזת ריקה = טופל. השולח בודק בדיוק את זה לפני שהוא שולח שוב, אז
        // בלי השורה הזאת אותו פריט היה נחסם לנצח אחרי הלחיצה הראשונה.
        // הפרסום הוא היוצא מן הכלל: הוא מצב שאפשר להפוך, לא החלטה סופית.
    ...
    await tg(token, 'answerCallbackQuery', { callback_query_id: cq.id, text: msg.slice(0, 200) });
