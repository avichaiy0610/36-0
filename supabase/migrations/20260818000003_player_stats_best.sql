-- player_stats gains the best season itself, so the profile can offer "see the
-- XI that did it". Best = most points, ties broken by the stronger squad.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'best',            CASE WHEN b.id IS NULL THEN NULL ELSE jsonb_build_object(
                         'result_id', b.id, 'points', b.points, 'wins', b.wins,
                         'draws', b.draws, 'losses', b.losses, 'ovr', b.ovr,
                         'tier', b.tier, 'formation', b.formation,
                         'gf', b.gf, 'ga', b.ga,
                         'created_at', b.created_at, 'settings', b.settings,
                         'has_squad', EXISTS (SELECT 1 FROM squads s WHERE s.result_id = b.id)
                       ) END
  )
  INTO res
  FROM (SELECT 1) _
  ...
  LEFT JOIN (
    SELECT g.* FROM game_results g WHERE g.user_id = uid
    ORDER BY g.points DESC, g.ovr DESC, g.created_at DESC LIMIT 1
  ) b ON true;
